...
screen_columns <- screening_scenarios$screen_column %>% setdiff("none")
...
make_temperature_observations <- function(data, screen_column) {
  if (screen_column != "none") data <- data %>% filter(!.data[[screen_column]])
...
fit_temperature_scenario <- function(scenario_id, scenario_label,
  ...
  model <- prediction_grid %>%
    mutate(
      ...
      n_excluded_horizons = if (screen_column == "none") 0L else
...
temp_obs <- primary_fit$observations
...
CFB_soilcarb_with_temperature <- CFB_soilcarb_isotope_summary %>%
  mutate(
    exclude_from_temp_model = if (selected_screen_column == "none") FALSE else
      .data[[selected_screen_column]],
